...
This lecture note presents a brief introduction to the development environment and practices used in this course, namely:
...
The process that takes source code and external dependencies and produces the project deliverables is called the _build process_.
It takes as input:
...
For instance `implementation("org.slf4j:slf4j-api:2.0.16")` states that:
...
Gradle organizes dependencies by _dependency configurations_, of which `implementation`, `runtimeOnly`, and `testImplementation` are three examples:
...
* `runtimeOnly` dependencies are required when running the application but not when compiling it. Typically then contain interface implementations that are discovered and loaded dynamically during the application execution.
...
* The use of an `import` statement in a Kotlin file, such as `import org.slf4j.Logger`.

* And the use of the `implementation("org.slf4j:slf4j-api:2.0.16")` statement in a build script.

The `import` statement just introduces an alias in the source file where it is located, allowing the `org.slf4j.Logger` class to be referenced by its `org.slf4j.Logger` simple name.
...
The `implementation` statement instructs Gradle to locate, download, and make a JAR available in the classpath, namely the JAR file containing the `org.slf4j.Logger` class.
...
** E.g. programmer A always inserts the `{` starting a block in a new line, while programmer B inserts the opening `{` at the end of the previous line. If programmer B fixes a problem in a function written by programmer A the difference may include multiple line changes that are only due to programmer B applying their stylistic preferences and which aren't related to problem being solved.
...
* Installing a Java 25 distribution (the `temurin` distribution in this case).
